...
| ---------------------- | ---------------------------------------------------------------------------------------- |
| Mute                   | Mute channels on master, AUX, FX or matrix source buses (incl. feedback where available) |
| Solo                   | Set SOLO for channels on master bus (incl. feedback)                                     |
| Set Fader Levels       | Set the level of faders on master, AUX, FX or matrix source buses                        |
| Change Fader Levels    | Relatively change the level of faders on master, AUX, FX or matrix source buses          |
| Fade transitions       | Perform fade transitions on all channels, including matrix sources                       |
| Set channel delay      | Set delay for all faders and master                                                      |
| Set/change PAN         | Set and relatively change PAN for channels on master, AUX or stereo matrix sources       |
...
| Matrix source control  | Control AUX, subgroup and master sources routed to a matrix (Ui24R only)                 |
